...
ACar::ACar()
{
    ...
    m_carMovementComponent = CreateDefaultSubobject<UCarMovementComponent>(TEXT("Car movement component"));
    bReplicates = false;

    NetComponent = CreateDefaultSubobject<UNetComponent>(TEXT("Net component"));
}
...
float ACar::GetVelocityMagnitude() const { return m_carMovementComponent->GetVelocityMagnitude(); }

void ACar::SetInput(const FVector2D& input)
{
    m_vMovementInput = input;
}
...
void ACar::Tick(float DeltaTime)
{
    ...
    NetComponent->SetInput(m_vMovementInput);
}
